...
:::tip[Shaky on CSS selectors?]
The **[CSS selectors reference](/en/reference/css-selectors/)** is a plain-language primer on `.class`, `#id`, `[attr=val]`, combinators, and the engines Playwright adds on top — handy whenever a raw `page.locator('…')` string here looks cryptic.
:::
...
`:has-text("…")` is Playwright's CSS extension for matching an element by the text inside it — handy inside a chain. Prefer `getByRole` / `filter` when you can; reach for `:has-text()` for quick text-based narrowing.
:::
