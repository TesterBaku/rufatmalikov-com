...
:::tip[New to git push, branches, or pull requests?]
This module assumes you can commit and push to GitHub. If `git add`, `git commit`, `git push`, or "pull request" are fuzzy, the **[Git & GitHub basics reference](/en/reference/git-github/)** covers the exact commands used here — including why `auth/` belongs in `.gitignore`.
:::
...
**Remember:** gate behaviour on `process.env.CI` — `forbidOnly`, a retry, a single worker — and let `webServer` start/await the app (`reuseExistingServer: !process.env.CI`); start the app once, never twice.
...
:::tip[New to Docker itself?]
This section runs your tests inside Docker. If `image`, `container`, `docker run`, or Compose are unfamiliar, the **[Docker basics reference](/en/reference/docker/)** walks through installing Docker and the core commands used here.
:::
