...
- `actions/checkout@v4` — clones the exact commit that triggered the workflow.
- `actions/setup-node@v4` — installs the specified Node version.
- Two `npm ci` commands — the app and the tests live in separate directories with separate `package.json` files. Both must be installed.
...
This creates four parallel jobs. Shard 1 runs the first quarter of the test suite, shard 2 the second quarter, and so on. Total wall-clock time drops by roughly 75%.

**Merging shard reports.** Each shard produces its own blob report. To merge them into a single HTML report:
...
- `-v $(pwd):/app` mounts your project directory into the container at `/app`.
- `-w /app/tests/final` sets the working directory inside the container.
...
The volume mount in the Compose file copies the `playwright-report` directory back to your host after the container exits, so you can open the HTML report locally.
...
- Why are there two separate `npm ci` commands?
...
- Checkout, Setup Node, Install app deps, Install test deps, Install browsers, Run tests, Upload report
