...
:::tip[Fuzzy on roles, the DOM, or semantic HTML?]
Accessibility auditing is all about elements having the right **role** and **accessible name**. If those terms (or "the DOM" itself) feel shaky, the **[DOM & HTML basics reference](/en/reference/dom-html/)** explains tags, the element tree, and how semantic HTML maps to the roles axe checks.
:::
...
:::tip[New to WCAG, ARIA, or what axe actually checks?]
The **[Accessibility, ARIA & WCAG reference](/en/reference/accessibility/)** covers the POUR principles, the ARIA attributes, the axe rule ids you'll see in `violations` (`color-contrast`, `image-alt`, `label`…), and why automation only catches ~30–40% of issues.
:::
