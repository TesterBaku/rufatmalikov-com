...
These exercises use the local TestMarket app. Start it with `npm start` in the `capstone-app/` directory. Run your tests from the `tests/starter/` directory.

---

### Exercise 1: Build LoginPage

Create `tests/starter/pages/LoginPage.ts`:
...
Create `tests/starter/pages/BasePage.ts`:
...
Create `tests/starter/pages/ProductsPage.ts`:
...
Create `tests/starter/pages/CartPage.ts`:
...
Create `tests/starter/pages/CheckoutPage.ts`:
...
5. Places the order.
6. Asserts redirect to `/order-confirmation`.

---

### Exercise 6: Build AdminUsersPage

The admin panel at `/admin/users` lists all registered users. Create `tests/starter/pages/AdminUsersPage.ts`:
...
Create `tests/starter/pages/PageManager.ts` that accepts `page` in its constructor and exposes:
...
Compare your final `pages/` directory with the reference solution at `tests/final/pages/`.
