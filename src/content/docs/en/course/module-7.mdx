...
- Locators (the cart page has no per-item testids — use these): `cartItems` → `.cart-table .table-row`, `removeButtons` → `.btn-danger`, `checkoutButton` → `a[href="/checkout"]`, `emptyCartMessage` → `page.getByText('Your cart is empty')` (there is **no** `.cart-empty` class).
