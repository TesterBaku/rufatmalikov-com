...
These exercises run on plain Node.js — no browser needed. Open VS Code, set up the files below, and run them with `npx tsx exercises/mod1/exercise-X.ts`. If you don't have `tsx` yet, install it once with `npm install -D tsx`.

---

### Exercise 1: Variables — fix the mistakes

The code below has 4 bugs. Fix them and run the file.
...
Make `exercise-2.ts` with these four arrow functions:
...
### Exercise 3: Build test data objects
...
```typescript
const fakePage = {
  goto: (url: string) => Promise.resolve(`Navigated to ${url}`),
  fill: (selector: string, value: string) => Promise.resolve(`Filled ${selector} with ${value}`),
  click: (selector: string) => Promise.resolve(`Clicked ${selector}`),
  waitForSelector: (selector: string) => Promise.resolve(`Waited for ${selector}`),
