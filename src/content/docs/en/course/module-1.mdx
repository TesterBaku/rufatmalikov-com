...
// 1. Build validAdmin from validCustomer with the spread operator,
//    overriding only email and role.
...
// 2. Get the names of all 'accessories' products (filter + map).
// TODO: const accessoryNames = products.filter(p => p.category === 'accessories').map(p => p.name);
...
Write these five arrow functions in `exercise-3.ts`:
