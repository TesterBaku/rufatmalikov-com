...
def api():
    ...
    session.base_url = "http://localhost:3000"
...
def test_products_with_session(api):
    response = api.get(f"{api.base_url}/api/products")
    assert response.status_code == 200
```
...
def reset_db(base_url):
    """Restore TestMarket Lab to its seed state before the test runs."""
    requests.post(f"{base_url}/api/reset")
    yield
    # (optional) reset again after, so the next test starts clean too
    requests.post(f"{base_url}/api/reset")
```
...
creates a product via `POST /api/products`, then a **second** test that asserts the product
...
Give your login parametrize readable case names with the `ids=` argument
(e.g. `ids=["valid", "bad-password", "missing-fields"]`) so the output reads
`test_login_states[valid]` instead of the raw values. Check `pytest -v`.
