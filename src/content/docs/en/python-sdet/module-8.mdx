...
Put the cases in a JSON file, load it **at module level** (the `parametrize` decorator runs at
import time, so the data must exist then), and parametrize over the list:
...
@pytest.mark.parametrize("case", CASES, ids=[c["email"] or "empty" for c in CASES])
...
directory. Each JSON object is one case; the test body never changes when you add rows.
...
CSV suits tabular cases a spreadsheet could produce. `csv.DictReader` yields one dict per row —
but every value is a **string**, so convert types yourself:
...
test_login_labelled[empty->400] PASSED
```

**Remember:** build `ids=` from each case's data so failures name themselves
(`[empty->400]`) instead of showing an opaque dict — you see *which* case failed at a glance.
