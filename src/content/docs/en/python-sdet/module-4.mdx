...
# strict=True: when the bug is fixed, this XPASSes and FAILS the run, so the
# stale marker can't linger. Keep the marker focused on the one buggy assertion.
...
`conftest.py` to stay re-runnable. The arrange `POST` isn't asserted — only the duplicate is,
so the `xfail` stays scoped to the actual bug and won't mask an unrelated regression.

**Remember:** capture a known bug as a **`strict=True`** `xfail` test that asserts the
*correct* behavior with a `reason`, keeping the marker on just the buggy assertion. It
documents the defect, keeps the suite green today, and **fails loudly the moment the fix
lands** — far better than a comment or a deleted test.
