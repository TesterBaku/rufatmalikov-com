...
tests/test_failure_demo.py:4: AssertionError
```
...
small; Module 3 goes deep on API testing.
...
Work in your `python-sdet` project with TestMarket Lab running. Create the files under
...
- Assert that `int("abc")` raises `ValueError`, and that the message contains
  `"invalid literal"`.
- Assert that `0.1 + 0.2 == pytest.approx(0.3)`.
- Assert that the three prices `29.99 + 89.99 + 24.99` equal `pytest.approx(144.97)`.
...
### Exercise 5 — Products API checks (15 min)

In `tests/test_products_api.py`, write tests that:
- `GET /api/products` → status `200` and a non-empty list.
- Every product has `id`, `name`, and `price`.
- `GET /api/products?category=accessories` → every result has `category == "accessories"`.
