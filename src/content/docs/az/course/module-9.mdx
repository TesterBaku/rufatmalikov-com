...
Playwright daxili HTTP müşteri ilə gəlir. İstənilən testdə `request`-i destructuring edərək alırsınız:
...
test('POST /api/products məhsul yaradır', async ({ request }) => {
  const res = await request.post('/api/products', {
    data: { name: 'Klaviatura', price: 49.99, category: 'elektronika' },
  });
...
Bu nümunə xəta sərhədlərini, ehtiyat UI-larını və yenidən cəhd məntiqini test etmək üçün vacibdir — test mühitində real 500-ü etibarlı şəkildə tetikləmək mümkün deyil.
...
test('products API-ni gözləyib yoxlayır', async ({ page }) => {
  // Sorğunu tetikləyən əməliyyatdan ƏVVƏL dinləməyə başlayın
...
`waitForResponse` vədini həmişə sorğunu tetikləyən əməliyyatdan **əvvəl** qurun. Sonra qursanız, cavab artıq gəlib keçmiş ola bilər və sonsuz gözləyəcəksiniz.
...
- `GET /api/products?category=elektronika` → bütün nəticələr `category: 'elektronika'`-ya malikdir
