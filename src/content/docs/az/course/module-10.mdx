...
:::tip[Cookie-lər, sessiyalar və ya `HttpOnly` qaranlıqdır?]
Bu modul brauzerin necə sistemə daxil qaldığına əsaslanır. **[Cookie-lər və sessiyalar istinadı](/az/reference/cookies-sessions/)** cookie-ləri, server tərəfli sessiyaları, `connect.sid`-i və `storageState`-in niyə `HttpOnly` cookie-ni tuta *bildiyini* izah edir — bu modulun ifşa etdiyi mif.
:::
