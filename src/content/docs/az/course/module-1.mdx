...
// 1. validAdmin-i validCustomer-dən spread operatoru ilə qur,
//    yalnız email və role-u dəyiş.
// TODO: const validAdmin = { ...validCustomer, email: 'admin@test.io', role: 'admin' };
...
// 2. Bütün 'aksesuarlar' məhsullarının adlarını al (filter + map).
// TODO: const accessoryNames = products.filter(p => p.category === 'aksesuarlar').map(p => p.name);

// 3. validAdmin-dən email və role-u destructure et.
// TODO: const { email, role } = validAdmin;
