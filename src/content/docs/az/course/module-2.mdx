...
:::tip[CSS seçicilərində özünə əmin deyilsən?]
**[CSS seçiciləri istinadı](/az/reference/css-selectors/)** `.class`, `#id`, `[attr=val]`, kombinatorlar və Playwright-in əlavə etdiyi mühərriklər haqqında sadə dildə primerdir — buradakı xalis `page.locator('…')` sətri qəliz görünəndə əlverişlidir.
:::
...
:::note[`:has-text()` nədir?]
`:has-text("…")` — Playwright-in CSS genişlənməsidir, elementi içindəki mətnə görə uyğunlaşdırır — zəncirdə əlverişlidir. Mümkün olduqda `getByRole` / `filter` üstün tut; tez mətn əsaslı daralma üçün `:has-text()`-ə müraciət et.
:::
