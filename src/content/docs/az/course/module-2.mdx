...
<Practice missions={['first-click','get-by-label','get-by-placeholder','find-by-text','test-id','chained-locators','filter-sale-card','filter-has-locator','nth-first-last','dynamic-ids','table-row-by-text','shadow-dom']} />
...
<Practice missions={['role-with-name','exact-match','strict-mode-disambiguation','nth-vs-first','iterate-multiple','disambiguation-cookbook']} />
...
Bu tapşırıqlar öz kompüterinizdə işlədəcəyiniz kiçik e-ticarət tətbiqi olan **TestMarket Lab**-a qarşı işləyir. Onu kursun qalan hissəsində də istifadə edəcəksiniz, ona görə indi bir dəfə quraşdırın (Node.js 20+ tələb olunur, [Module 0](/az/course/module-0/)-da quraşdırılıb):
...
Onu işlək vəziyyətdə saxlayın və brauzerdə **http://localhost:3000** ünvanını açın. Tapşırıq daxil olmuş istifadəçi tələb etdikdə demo hesablardan istifadə edin:

- **Müştəri** — `customer@test.io` / `customer123`
- **Admin** — `admin@test.io` / `admin123`

> Təmiz başlanğıc lazımdır? Tətbiqi yenidən başladın və ya seed məlumatlarını bərpa etmək üçün `POST http://localhost:3000/api/reset` göndərin.
