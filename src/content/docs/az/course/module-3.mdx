...
```javascript
// type() — real klaviatura zərbələrini simulyasiya edir, simvol-simvol
...
**`type()` nə vaxt lazımdır:** React-Select, typeahead komponentlər, zəngin mətn redaktorları və ya yekun dəyər əvəzinə hər bir ayrı klaviatura zərbəsinə reaksiya verən istənilən daxiletmə.

**Tuz:** Sahədə artıq mətn varsa, `type()` ona əlavə edir. `fill()` isə həmişə əvvəlcə təmizləyir.

```javascript
// Sahə "old value" ilə başlayır — type() əlavə edir:
await page.locator('#name').type(' new suffix'); // nəticə: "old value new suffix"
...
**Capstone tətbiqi nümunəsi — məhsulları filtrləyin:**
...
- Menyu animasiya ilə açılır? Tetikleyici click-dən sonra `await page.waitForTimeout(300)` əlavə edin
- Seçenek viewport xaricindədir? Click-dən əvvəl scroll edin
- Click etmədən əvvəl menyu bağlanır? Seçenekdə `{ force: true }` istifadə edin

```javascript
// Click-away davranışı müdaxilə etdikdə seçeneyi force click edin
...
### Tapşırıq 1: Login forması — `fill()` vs `type()`
...
test.describe('Login form actions', () => {
  ...
  test('type() — simvol-simvol', async ({ page }) => {
    await page.goto('/auth/login');
    await page.locator('#email').type('customer@test.io', { delay: 30 });
...
**Tapşırıqlar:**
- `npx playwright test --headed` ilə işlədin və yazma sürətini müşahidə edin: `fill()` anlıqdır, `type()` simvol-simvol yazır
- `{ delay: 30 }` şərhə alın və yenidən işlədin — nə dəyişir?
- Əvvəlcədən doldurulmuş sahədə əvvəlcə təmizləmədən `type()` istifadə edərək əlavə etmə davranışını görün

---

### Tapşırıq 2: Checkout forması — doldur və göndər

`tests/starter/tests/actions/checkout-actions.spec.js` yaradın. `beforeEach` daxil olur, səbətə məhsul əlavə edir və `/checkout`-a keçir.
...
test('sahələri Tab-la keçin və type() ilə dəyər daxil edin', async ({ page }) => {
...
**İstinad:** `tests/final/pages/CheckoutPage.js`
...
### Tapşırıq 4: Tarix daxiletməsi və klaviatura qısayolları

`tests/starter/tests/actions/date-input-actions.spec.js` yaradın:

```javascript
test('mətn daxiletməsinə tarix formatı doldurun', async ({ page }) => {
...
test('path vasitəsilə fayl yükləyin', async ({ page }) => {
  ...
  // Fayl daxiletməsini inject edin (capstone register səhifəsindəki yoxdur)
  await page.evaluate(() => {
...
1. `fill()` ilə `type()` arasındakı fundamental fərq nədir?
...
1. **Formlar üçün `fill()`, xüsusi hallar üçün `type()`** — `fill()` daha sürətlidir, həmişə əvvəlcə təmizləyir və daxiletmələrin 95%-i üçün düzgün hadisələri işə salır
