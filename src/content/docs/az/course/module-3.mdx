...
:::note[`:has-text()` nədir?]
`:has-text("…")` — Playwright-in CSS genişlənməsidir, elementi içindəki mətnə görə uyğunlaşdırır. `getByRole('button', { name: 'Apply Filters' })` adətən daha təmizdir; `:has-text()` əlverişli ehtiyat variantdır.
:::
