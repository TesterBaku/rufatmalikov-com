...
:::tip[Terminal, npm və ya npx ilə yeni tanışsan?]
Bu quraşdırma `npm install`, `npm start` və `npx playwright test` kimi əmrlər istifadə edir. Terminal yeni ərazidirsə, **[Komanda sətri və npm istinadı](/az/reference/command-line-npm/)** `cd`/`ls`-i, `package.json` və `node_modules/`-un nə olduğunu və `npm install` ilə `npx` arasındakı fərqi izah edir — məhz bu əmrlərlə.
:::
...
Onu işlək saxla və brauzerdə [http://localhost:3000](http://localhost:3000) ünvanını aç — TestMarket mağazasını görməlisən.

Sonra **test** layihəni ona yönəlt — `playwright.config.ts`-ə `baseURL` əlavə et:
...
:::tip[Məlumatları istənilən vaxt sıfırla]
TestMarket Lab kiçik yerli verilənlər bazası istifadə edir. Tapşırıqlar arasında ilkin vəziyyətə qaytarmaq üçün tətbiqin qovluğunda `npm run reset` işlət (və ya işləyərkən `/api/reset`-ə `POST` göndər).
:::

**İki cür məşq:** [BrauzerLab](https://brauzerlab.rufatmalikov.com/az/) brauzerində heç nə quraşdırmadan işləyir — sürətli məşqlər üçün; **TestMarket Lab** isə yerli işləyir və ona qarşı tam test dəstləri yazırsan.
