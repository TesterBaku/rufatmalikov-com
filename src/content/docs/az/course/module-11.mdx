...
:::tip[Rollar, DOM və ya semantik HTML qaranlıqdır?]
Əlçatımlılıq auditi tamamilə elementlərin düzgün **rola** və **əlçatan ada** malik olması haqqındadır. Bu terminlər (ya da "DOM"-un özü) qeyri-müəyyəndirsə, **[DOM və HTML əsasları istinadı](/az/reference/dom-html/)** teqləri, element ağacını və semantik HTML-in axe-in yoxladığı rollara necə uyğunlaşdığını izah edir.
:::
...
**Yadda saxla:** `...devices['iPhone 13']`-i yaymaq sadəcə ölçü dəyişikliyi deyil — viewport, DPR, user-agent, `hasTouch` və `isMobile`-i təyin edir, beləcə klik əvəzinə toxunma hadisələri atəşlənir və `pointer: coarse` media sorğuları aktivləşir.
...
:::tip[WCAG, ARIA və ya axe-in nəyi yoxladığı ilə yeni tanışsan?]
**[Əlçatımlılıq, ARIA və WCAG istinadı](/az/reference/accessibility/)** POUR prinsiplərini, ARIA atributlarını, `violations`-da görəcəyin axe qayda id-lərini (`color-contrast`, `image-alt`, `label`…) və avtomatlaşdırmanın niyə yalnız ~30–40% problemi tutduğunu əhatə edir.
:::
