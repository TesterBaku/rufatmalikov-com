...
:::tip[git push, branch və ya pull request-lərlə yeni tanışsan?]
Bu modul GitHub-a commit və push edə bildiyini güman edir. `git add`, `git commit`, `git push` və ya "pull request" qaranlıqdırsa, **[Git və GitHub əsasları istinadı](/az/reference/git-github/)** buradakı dəqiq əmrləri əhatə edir — `auth/`-un niyə `.gitignore`-da olduğu da daxil.
:::
...
:::tip[Docker-in özü ilə yeni tanışsan?]
Bu bölmə testlərinizi Docker-in içində işlədir. `image`, `container`, `docker run` və ya Compose tanış deyilsə, **[Docker əsasları istinadı](/az/reference/docker/)** Docker-in quraşdırılmasını və buradakı əsas əmrləri əhatə edir.
:::
