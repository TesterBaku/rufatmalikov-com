...
Bu modulun sonunda hər push və pull request-də tetiklənən, brauzerləri quraşdıran, testlər dəstini təmiz Linux mühitində işlədən, HTML hesabatını yüklənə bilən artefakt kimi yayımlayan və isteğe bağlı olaraq hər şeyi rəsmi Playwright Docker konteynerinin içində işlədən bir workflow-a sahib olacaqsınız.
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Tətbiq asılılıqlarını quraşdır
        working-directory: capstone-app
        run: npm ci

      - name: Test asılılıqlarını quraşdır
        working-directory: tests/final
        run: npm ci
      ...
      - name: Test hesabatını yüklə
        ...
        with:
          ...
          path: tests/final/playwright-report/
          retention-days: 7
```

**Workflow-un quruluşu:**

- `on` — tetikləmə hadisələri. Bu workflow `main`-ə hər push-da və `main`-i hədəf alan hər pull-request-də işləyir.
- `runs-on: ubuntu-latest` — hər işlətmə üçün təzə Linux virtual maşını hazırlanır. Əvvəlki işlətmədən heç nə qalmır.
- `actions/checkout@v4` — workflow-u tetikləyən commit-i klonlayır.
- `actions/setup-node@v4` — göstərilən Node versiyasını quraşdırır.
- İki ayrı `npm ci` əmri — tətbiq və testlər ayrı `package.json` faylları olan ayrı qovluqlarda yerləşir. Hər ikisi quraşdırılmalıdır.
...
- `workers: 1` — tək maşında tətbiq serveri və çoxsaylı brauzer prosesləri port münaqişəsinin qarşısını alır.
...
Bu, dörd paralel iş yaradır. Shard 1 test dəstinin ilk dörddə birini, shard 2 ikinci dörddə birini işlədir və bu şəkildə davam edir. Ümumi işlətmə müddəti təxminən 75% azalır.

**Shard hesabatlarını birləşdirmək.** Hər shard öz blob hesabatını hazırlayır. Onları tək HTML hesabatında birləşdirmək üçün:
...
- Uğursuzluq zamanı çəkilmiş sıradan görüntüler və videolar
...
- `-v $(pwd):/app` layihə qovluğunuzu konteynerin içindəki `/app`-a qoşur.
- `-w /app/tests/final` konteynerin içindəki iş qovluğunu təyin edir.
...
COPY tests/final/package.json tests/final/package-lock.json ./tests/final/
COPY capstone-app/package.json capstone-app/package-lock.json ./capstone-app/

RUN cd capstone-app && npm ci
RUN cd tests/final && npm ci
# Brauezrlər artıq əsas obrazda quraşdırılmışdır — əlavə quraşdırma addımı tələb olunmur
...
services:
  tests:
    ...
    volumes:
      - ./tests/final/playwright-report:/app/tests/final/playwright-report
```
...
Compose faylındakı volume qoşulması konteyner çıxdıqdan sonra `playwright-report` qovluğunu yerinizə kopyalayır ki, HTML hesabatını lokalda aça biləsiniz.
...
- Workflow-u hansı hadisələr tetikləyir?
- Niyə iki ayrı `npm ci` əmri var?
...
- Checkout, Setup Node, Tətbiq asılılıqlarını quraşdır, Test asılılıqlarını quraşdır, Brauzerləri quraşdır, Testləri işlət, Hesabatı yüklə
